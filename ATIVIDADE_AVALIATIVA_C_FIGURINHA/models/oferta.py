...
class OfertaTroca(ModeloBase):
    ...
    def listar_com_colecionador(cls):
        return cls.query.order_by(cls.data_criacao).all()

    
class ItemOferta(ModeloBase):
    __tablename__ = "itens_oferta"

    oferta_id = db.Column(db.Integer, db.ForeignKey("ofertas_troca.id"), nullable=True)
    figurinha_id = db.Column(db.Integer, db.ForeignKey("figurinhas.id"), nullable=True)
